      ...
      <header className="sticky top-4 z-[50] w-full px-4 md:px-6 mt-2 mb-4">
        ...
        >
          <div className="px-4 md:px-6 h-[72px] flex items-center justify-between relative">
            ...
            <NavLink
              to="/"
              className="flex items-center gap-3 group mr-8 relative z-20"
              aria-label="Coffee and Co Home"
            >
              <motion.div
            ...
            <div className="hidden md:flex justify-center flex-1 relative z-30">
              <NavigationMenu>
                <NavigationMenuList>
                  <NavigationMenuItem>
                    <NavigationItemDirect to="/" end>
                      {t("nav.home")}
                    </NavigationItemDirect>
                  </NavigationMenuItem>

                  <NavigationMenuItem>
                    <NavigationItemDirect to="/menu">
                      {t("nav.menu")}
                    </NavigationItemDirect>
                  </NavigationMenuItem>
                          ...
                          <li>
                            <NavigationMenuLink to="/about" end>
                              <div className="flex items-start gap-3">
                                ...
                                <div>
                                  <span className="block text-sm font-bold text-coffee-900 dark:text-coffee-100">
                                    {t("nav.ourStory")}
                                  </span>
                                  <span className="block text-xs text-coffee-500 dark:text-coffee-400 mt-0.5">
                                    {t("nav.ourStoryDesc")}
                                  </span>
                                </div>
                              </div>
                  ...
                  <NavigationMenuItem>
                    <NavigationItemDirect
                      to={isAuthenticated ? "/rewards" : "/login"}
                    >
                      {t("nav.rewards")}
                    </NavigationItemDirect>
                  </NavigationMenuItem>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `relative p-2 rounded-full transition-all group hidden sm:block ${
                        isActive
                          ? "text-error bg-error/10 dark:bg-error/10 dark:text-error"
                          : "text-coffee-700 dark:text-coffee-200 hover:bg-coffee-100 dark:hover:bg-white/10"
                      }`
                    }
                  ...
                  >
                    ...
                    {isAuthenticated && favoriteCount > 0 && (
                      <motion.span
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className="absolute top-1.5 right-1.5 h-2 w-2 bg-error rounded-full border border-white dark:border-coffee-900"
                      />
                    )}
                  </NavLink>
                </TooltipTrigger>
                      ...
                      <NavigationMenuItem value="profile">
                        ...
                        <NavigationMenuContentWrapper value="profile">
                          <NavigationMenuContent className="w-[220px] -right-2 left-auto p-2">
                            <ul className="grid gap-1">
                              {["admin", "superadmin", "barista"].includes(
                                user.role
                              ) && (
                                <li>
                              ...
                              <li>
                                <AlertDialog>
                                  <AlertDialogTrigger asChild>
                                    <button className="relative block w-full select-none space-y-1 rounded-3xl p-3 leading-none no-underline outline-none transition-colors text-left font-medium text-error hover:bg-error/10 hover:text-error">
                                      <div className="flex items-center gap-2">
          ...
          <AnimatePresence>
            {isMobileMenuOpen && (
              ...
              >
                <nav
                  className="flex flex-col p-5 pt-0 gap-2 pb-8"
                ...
                >
                  ...
                  <NavLink
                    ...
                    className="text-lg font-medium py-3 px-6 rounded-[2rem] text-coffee-600 dark:text-coffee-300 hover:bg-white/40 dark:hover:bg-coffee-800"
                  >
                    {t("nav.home")}
                  </NavLink>
                  <NavLink
                  ...
                  {isAuthenticated && (
                    <div className="py-2 px-6 mt-2">
                      <button
                        onClick={() =>
                          setActiveMobileSection(
                            activeMobileSection === "profile" ? null : "profile"
                          )
                        }
                        className="flex items-center justify-between w-full text-lg font-medium text-coffee-900 dark:text-coffee-100 mb-2"
                      >
                            ...
                            <div className="pl-4 border-l-2 border-coffee-100 dark:border-coffee-800 space-y-2 pb-2">
                              {["admin", "superadmin", "barista"].includes(
                                ...
                                <NavLink
                                  to="/dashboard"
                                  onClick={() => setIsMobileMenuOpen(false)}
                                  className="block py-1 font-bold text-coffee-700 dark:text-coffee-300"
                                >
                                  {t("common.dashboard")}
                                </NavLink>
                              )}
                              ...
                              <button
                                ...
                                className="block py-1 text-error"
                              >
                                {t("nav.signOut")}
                              </button>
                            </div>
