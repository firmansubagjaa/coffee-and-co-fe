...
import { ShieldCheck, Lock, CreditCard, Loader2, Wallet, QrCode, Building, CheckCircle, ChevronRight, AlertTriangle, ShoppingBag, MapPin, ArrowLeft } from 'lucide-react';
          ...
          case 'bank_transfer':
              return (
                  <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                      <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-2xl border border-blue-100 dark:border-blue-800 flex items-center justify-between shadow-sm">
                          <div>
                              <p className="text-xs text-blue-600 dark:text-blue-400 font-bold uppercase tracking-wider mb-2">BCA Virtual Account</p>
                              <p className="text-3xl font-mono font-bold text-blue-900 dark:text-blue-100 tracking-widest">8801 2345 6789 000</p>
                          </div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-coffee-400 leading-relaxed">{t('checkout.payment.instruction.transfer')}</p>
                  </div>
              );
          case 'gopay':
          case 'qris':
                  ...
                  <div className="flex flex-col items-center justify-center space-y-6 py-6 animate-in fade-in zoom-in">
                      <div className="bg-white p-6 rounded-3xl border-2 border-gray-100 dark:border-coffee-800 shadow-lg relative">
                          ...
                          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white px-3 py-1 rounded-full shadow-sm border border-gray-100">
                              <span className="text-[10px] font-bold uppercase tracking-widest text-gray-500">Scan QR</span>
                          </div>
                      </div>
                      <p className="text-sm font-medium text-gray-900 dark:text-white bg-gray-100 dark:bg-coffee-800 px-4 py-2 rounded-full">{t('checkout.payment.instruction.scan')}</p>
                  </div>
              );
          case 'credit_card':
              return (
                  <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                      <div className="p-6 border rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 text-white shadow-xl relative overflow-hidden">
                          ...
                          <p className="font-mono text-2xl tracking-widest mb-6">0000 0000 0000 0000</p>
                      ...
                      <p className="text-xs text-center text-gray-500 dark:text-coffee-400 mt-2">{t('checkout.payment.instruction.card')}</p>
                  </div>
              );
          default:
              return null;
      ...
      return (
        <div className="min-h-screen bg-cream-50 dark:bg-coffee-950 flex items-center justify-center p-4">
            ...
            >
                ...
                <h2 className="text-3xl font-serif font-bold text-coffee-900 dark:text-white mb-4">{t('common.success')}</h2>
                <p className="text-coffee-600 dark:text-coffee-300 mb-10 text-lg">{t('checkout.payment.successDesc')}</p>
                
                <Button fullWidth onClick={handleFinish} size="lg" className="rounded-full shadow-xl hover:shadow-2xl transition-all hover:-translate-y-1">
                    ...
                    return (
                        <div key={s.id} className="flex items-center gap-4">
                            <div className={cn(
                                ...
                                isActive ? "bg-coffee-900 text-white dark:bg-white dark:text-coffee-900 shadow-lg" : 
                                isPast ? "text-coffee-900 dark:text-white opacity-50" : "text-coffee-300 dark:text-coffee-700"
                            )}>
                                <span className={cn(
                                    "w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold",
                                    isActive ? "bg-white text-coffee-900 dark:bg-coffee-900 dark:text-white" : 
                                    isPast ? "bg-coffee-200 dark:bg-coffee-800" : "bg-coffee-100 dark:bg-coffee-800"
                                )}>
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-16">
            ...
            <div className="flex-1">
                <div className="flex items-center gap-4 mb-8">
                    <button onClick={() => navigate('/cart')} className="p-2 hover:bg-coffee-100 dark:hover:bg-coffee-800 rounded-full transition-colors">
                    ...
                    <h1 className="text-3xl md:text-4xl font-serif font-bold text-coffee-900 dark:text-white">
                        {step === 'method' ? t('checkout.payment.title') : 'Complete Payment'}
                    </h1>
                </div>
                ...
                >
                    ...
                    {step === 'method' && (
                        <div className="space-y-4 relative z-10">
                            <p className="text-coffee-500 dark:text-coffee-400 mb-6">Select your preferred payment method to proceed.</p>
                            ...
                            {[
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <span className="font-bold text-lg text-coffee-900 dark:text-white group-hover:text-coffee-700 dark:group-hover:text-coffee-100">{method.label}</span>
                                    </div>
                    ...
                    {step === 'instruction' && (
                        <div className="relative z-10">
                            <div className="flex items-center justify-between mb-8 bg-coffee-50 dark:bg-coffee-800 p-4 rounded-2xl">
                                <button onClick={() => setStep('method')} className="text-sm text-coffee-600 dark:text-coffee-300 hover:text-coffee-900 dark:hover:text-white font-bold flex items-center gap-2">
                                    <ArrowLeft className="w-4 h-4" /> {t('checkout.payment.changeMethod')}
                                </button>
                                <div className="text-right flex items-center gap-3">
                                    <p className="text-[10px] uppercase text-coffee-400 font-bold tracking-widest hidden sm:block">{t('checkout.payment.expiresIn')}</p>
                                    <div className="font-mono text-red-500 font-bold bg-white dark:bg-coffee-900 px-3 py-1 rounded-lg shadow-sm border border-red-100 dark:border-red-900/30">
                            ...
                            <div className="bg-yellow-50 dark:bg-yellow-900/10 p-4 rounded-xl flex gap-3 items-start mb-8 border border-yellow-100 dark:border-yellow-900/30">
                                ...
                                <p className="text-sm text-yellow-700 dark:text-yellow-200 leading-relaxed font-medium">{t('checkout.payment.instruction.simulation')}</p>
                            </div>

                            <Button fullWidth onClick={handleSimulatePayment} size="lg" className="rounded-full shadow-xl hover:shadow-2xl transition-all hover:-translate-y-1">
                    ...
                    {step === 'verifying' && (
                        <div className="py-20 text-center relative z-10">
                            ...
                            <h3 className="text-2xl font-bold text-coffee-900 dark:text-white mb-3">{t('checkout.payment.verifying')}</h3>
                            <p className="text-coffee-500 dark:text-coffee-400">{t('checkout.payment.doNotClose')}</p>
                        </div>
                    )}
                </motion.div>
            </div>
                ...
                <div className="sticky top-32">
                    <div className="bg-white dark:bg-coffee-900 rounded-[2.5rem] p-8 shadow-lg border border-coffee-100 dark:border-coffee-800">
                            ...
                            {items.map((item) => (
                                <div key={item.id} className="flex gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="font-bold text-coffee-900 dark:text-white text-sm truncate">{item.name}</p>
                                        <p className="text-xs text-coffee-500 dark:text-coffee-400 mb-1">Qty: {item.quantity}</p>
                                        <p className="font-bold text-coffee-700 dark:text-coffee-200 text-sm">{CURRENCY}{(item.price * item.quantity).toFixed(2)}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="border-t border-coffee-100 dark:border-coffee-800 pt-6 space-y-3">
                            ...
                            <div className="flex justify-between text-sm text-coffee-600 dark:text-coffee-300">
                                <span>Shipping</span>
                                <span className="font-medium">{shippingCost === 0 ? 'Free' : `${CURRENCY}${shippingCost.toFixed(2)}`}</span>
                            </div>
