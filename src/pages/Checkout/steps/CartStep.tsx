  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-20">
        <p className="text-coffee-500 dark:text-coffee-400 mb-6 text-lg">{t('cart.empty')}</p>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-white dark:bg-coffee-900 rounded-[2rem] p-6 md:p-8 shadow-sm border border-coffee-100 dark:border-coffee-800">
        <h2 className="text-2xl font-serif font-bold text-coffee-900 dark:text-white mb-6">{t('cart.title')}</h2>
        
        <div className="space-y-6">
          {items.map((item) => (
            <div key={item.id} className="flex gap-4 md:gap-6 pb-6 border-b border-coffee-100 dark:border-coffee-800 last:border-0 last:pb-0">
              ...
              <div className="flex-1 flex flex-col justify-between py-1">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-bold text-coffee-900 dark:text-white text-lg">{item.name}</h3>
                    <p className="text-sm text-coffee-500 dark:text-coffee-400 capitalize">{item.category}</p>
                  </div>
                ...
                <div className="flex items-center justify-between mt-4">
                  <div className="flex items-center bg-coffee-50 dark:bg-coffee-800 rounded-full h-10 px-1 border border-coffee-100 dark:border-coffee-700">
                    ...
                    <span className="w-8 text-center font-bold text-coffee-900 dark:text-white">{item.quantity}</span>
                    <button 
                      onClick={() => updateQuantity(item.id, item.quantity + 1)}
                      className="w-10 h-full flex items-center justify-center text-coffee-600 dark:text-coffee-300 hover:text-coffee-900 dark:hover:text-white transition-colors"
                    >
                  ...
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <button className="p-2 text-coffee-400 hover:text-red-500 transition-colors rounded-full hover:bg-red-50 dark:hover:bg-red-900/20">
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>{t('cart.removeDesc')}</AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
