...
export const RewardsPage: React.FC = () => {
  ...
  const nextTier = currentTier.nextTierId
    ? REWARD_TIERS[currentTier.nextTierId]
    : null;

  const pointsToNextTier = nextTier
    ? nextTier.minPoints - user.currentPoints
    : 0;
  const progressPercent = nextTier
    ? ((user.currentPoints - currentTier.minPoints) /
  ...
  const handleMysterySpin = () => {
    ...
    setTimeout(() => {
      ...
      toast.success(
        t("rewards.mystery.won", { reward: "50 " + t("rewards.points") })
      );
    }, 2000);
  };
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-7xl">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-coffee-500 dark:text-white/60">
              Unlock exclusive perks and enjoy free drinks.
            </p>
          </div>
          <div className="flex items-center gap-2 bg-white dark:bg-coffee-800 px-4 py-2 rounded-full shadow-sm border border-coffee-100 dark:border-white/10 self-start md:self-auto">
            ...
            <span className="font-bold text-base text-coffee-900 dark:text-white">
              {streak} Day Streak
            </span>
          </div>
        </div>
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-col md:flex-row md:items-end gap-4 md:gap-12 mb-8">
                <div>
                  <h2 className="text-6xl md:text-7xl font-bold tracking-tighter drop-shadow-md">
                    {user.currentPoints}
                  </h2>
              ...
              <div className="relative z-10 md:hidden bg-black/20 rounded-2xl p-4 backdrop-blur-md border border-white/10">
                <div className="flex justify-between text-xs font-bold mb-2">
                  <span className="text-white/90">
                    Next: {t(`rewards.tiers.${nextTier.id}`)}
                  </span>
            ...
            <div className="flex-1 bg-gradient-to-br from-indigo-600 to-purple-700 rounded-[2.5rem] p-8 relative overflow-hidden shadow-xl text-white flex flex-col justify-center items-center text-center group">
              ...
              <div className="relative z-10 w-full">
                ...
                <h3 className="font-bold text-2xl mb-2">
                  {t("rewards.mystery.title")}
                </h3>
                <p className="text-sm text-white/80 mb-6">
                  Spin daily for a chance to win bonus points and exclusive
                  coupons!
                </p>
                <Button
                  size="lg"
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <Button
              variant="ghost"
              className="text-coffee-600 dark:text-coffee-300 hover:bg-coffee-50 dark:hover:bg-coffee-800"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {REWARD_ITEMS.map((item, idx) => {
              const isLocked =
                item.minTierId &&
                Object.keys(REWARD_TIERS).indexOf(user.currentTierId) <
                  Object.keys(REWARD_TIERS).indexOf(item.minTierId);
              ...
              return (
                ...
                >
                    ...
                    <div
                      className={cn(
                        ...
                        isLocked
                          ...
                          : "bg-cream-100 dark:bg-black/20 group-hover:bg-coffee-900 group-hover:text-white dark:group-hover:bg-white dark:group-hover:text-coffee-900"
                      )}
                    >
                      {isLocked ? (
                        <Lock className="w-6 h-6 text-gray-400" />
                      ) : (
                        <item.icon className="w-8 h-8 text-coffee-600 dark:text-coffee-200 group-hover:text-white dark:group-hover:text-coffee-900" />
                      )}
                    </div>
                    {item.isPopular && (
                      <span className="bg-error/10 text-error text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wide">
                        Popular
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex-1">
                    <h4 className="font-bold text-lg text-coffee-900 dark:text-white mb-1 line-clamp-1">
                      {item.title}
                    </h4>
                    <p className="text-sm text-coffee-500 dark:text-white/60 font-medium mb-4">
                      {item.pointsCost} pts
                    </p>
                    ...
                    {isLocked ? (
                      ...
                      <Button
                        variant="outline"
                        className="w-full rounded-xl font-bold border-coffee-200 dark:border-white/10 hover:bg-coffee-900 hover:text-white dark:hover:bg-white dark:hover:text-coffee-900 transition-colors"
                      >
                        Redeem
                      </Button>
                    )}
