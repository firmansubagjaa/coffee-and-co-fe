...
const MOCK_TRANSACTIONS: Transaction[] = [
  ...
  },
  {
    ...
    amount: 24.0,
    status: "pending",
    date: "2024-05-24 10:42 AM",
    method: "Digital Wallet",
    items: "Cold Brew, Bagel, Muffin",
  },
  {
    id: "TXN-8824",
    customer: "David Lee",
...
export const TransactionsPage: React.FC = () => {
  ...
  const refundCount = MOCK_TRANSACTIONS.filter(
    (t) => t.status === "refunded"
  ).length;
  ...
  const handleExport = (format: string) => {
    ...
    toast.success(
      t("dashboard.transactions.exportSuccess", {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-coffee-900 p-6 rounded-2xl border border-coffee-100 dark:border-coffee-800 shadow-sm flex items-center gap-4">
          <div className="p-3 bg-success/10 dark:bg-success/20 rounded-xl text-success dark:text-success">
            <ArrowUpRight className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs font-bold text-coffee-400 uppercase tracking-wider">
              {t("dashboard.transactions.totalVolume")}
            </p>
        ...
        <div className="bg-white dark:bg-coffee-900 p-6 rounded-2xl border border-coffee-100 dark:border-coffee-800 shadow-sm flex items-center gap-4">
          <div className="p-3 bg-info/10 dark:bg-info/20 rounded-xl text-info dark:text-info">
            <CheckCircle2 className="w-6 h-6" />
          </div>
          <div>
            <p className="text-xs font-bold text-coffee-400 uppercase tracking-wider">
              {t("dashboard.transactions.successfulPayments")}
            </p>
            <h3 className="text-2xl font-bold text-coffee-900 dark:text-white">
              {successCount}
            </h3>
        ...
        <div className="bg-white dark:bg-coffee-900 p-6 rounded-2xl border border-coffee-100 dark:border-coffee-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-coffee-400 uppercase tracking-wider">
              {t("dashboard.transactions.refunds")}
            </p>
            <h3 className="text-2xl font-bold text-coffee-900 dark:text-white">
              {refundCount}
            </h3>
              ...
              <Button
                ...
                className={`capitalize whitespace-nowrap ${
                  filterStatus === status
                    ...
                    : "bg-white dark:bg-coffee-900 text-coffee-600 dark:text-coffee-300 border-coffee-200 dark:border-coffee-800"
                }`}
              >
      ...
      <div className="bg-white dark:bg-coffee-900 rounded-[2rem] border border-coffee-100 dark:border-coffee-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-coffee-100 dark:border-coffee-800">
          <h3 className="font-bold text-coffee-900 dark:text-white">
            {t("dashboard.transactions.showingRecent", {
            ...
            <thead className="bg-coffee-50/50 dark:bg-coffee-800/50 text-coffee-500 dark:text-coffee-400 font-bold uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4">
                  {t("dashboard.transactions.customer")}
                </th>
                <th className="px-6 py-4">
                  {t("dashboard.transactions.date")}
                </th>
                <th className="px-6 py-4">
                  {t("dashboard.transactions.amount")}
                </th>
                <th className="px-6 py-4">
                  {t("dashboard.transactions.payment")}
                </th>
                <th className="px-6 py-4">{t("dashboard.inventory.status")}</th>
                <th className="px-6 py-4 text-right">
                  {t("dashboard.inventory.action")}
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-coffee-50 dark:divide-coffee-800">
              {filteredTxns.length > 0 ? (
                filteredTxns.map((txn) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-coffee-400 truncate max-w-[150px]">
                        {txn.items}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-coffee-600 dark:text-coffee-400">
                      {t(
                        `dashboard.transactions.method.${getMethodKey(
                          txn.method
                        )}`
                      )}
                    </td>
                      ...
                      <Badge
                        variant={
                          txn.status === "completed"
                            ? "secondary" // Using secondary (greenish in this theme usually) or default
                            : txn.status === "failed"
                            ? "destructive"
                            : txn.status === "refunded"
                            ? "outline"
                        ...
                        className={`capitalize ${
                          txn.status === "completed"
                            ? "bg-success/10 text-success dark:bg-success/20 dark:text-success hover:bg-success/20"
                            : ""
                        }`}
                      >
                        {txn.status === "completed" && (
                ...
                <tr>
                  <td
                    colSpan={7}
                    className="px-6 py-12 text-center text-coffee-400"
                  >
