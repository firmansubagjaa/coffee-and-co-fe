...
export const AnalyticsPage: React.FC = () => {
  ...
  const handleExport = (format: string) => {
    ...
    toast.success(
      t("dashboard.analytics.exportSuccess", { format: format.toUpperCase() })
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3 items-center w-full md:w-auto">
          <Button
            variant="outline"
            className="bg-white dark:bg-coffee-900 gap-2 border-gray-200 dark:border-coffee-800 text-gray-600 dark:text-gray-300 h-11"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div
            key={i}
            className="bg-white dark:bg-coffee-900 p-8 rounded-[2rem] border border-gray-100 dark:border-coffee-800 shadow-sm hover:shadow-md transition-shadow"
          >
            <div className="flex justify-between items-start mb-6">
              <div
                className={`p-3.5 rounded-2xl ${m.bg} ${m.color} dark:bg-opacity-10`}
              >
              ...
              <span
                className={`flex items-center text-xs font-bold px-2.5 py-1 rounded-full ${
                  m.trend > 0 && !m.isBad
                    ? "text-success bg-success/10 dark:bg-success/20 dark:text-success"
                    : "text-success bg-success/10 dark:bg-success/20 dark:text-success"
                }`}
              >
            ...
            <div>
              ...
              <h3 className="text-4xl font-bold text-gray-900 dark:text-white tracking-tight">
                {m.title === t("dashboard.analytics.refundRate")
                  ? ""
                  : CURRENCY}
                {m.val}
              </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="h-72 flex items-end justify-between gap-2 sm:gap-4">
            {SALES_DATA.map((val, idx) => (
              ...
              >
                ...
                <p className="text-center text-xs text-gray-400 mt-4 font-bold uppercase tracking-wider">
                  {months[idx]}
                </p>
              </div>
            ))}
        ...
        <div className="bg-white dark:bg-coffee-900 p-10 rounded-[2.5rem] border border-gray-100 dark:border-coffee-800 shadow-sm">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-8">
            {t("dashboard.analytics.salesByCategory")}
          </h3>
          <div className="space-y-8">
            {[
              {
                name: t("common.categories.coffee"),
                pct: 45,
                color: "bg-coffee-800",
              },
              {
                name: t("common.categories.drinks"),
                pct: 30,
                color: "bg-coffee-600",
              },
              {
                name: t("common.categories.pastry"),
                pct: 15,
                color: "bg-yellow-500",
              },
              {
                name: t("common.categories.merch"),
                pct: 10,
                color: "bg-gray-300",
              },
            ].map((cat, i) => (
      ...
      <div className="bg-white dark:bg-coffee-900 p-10 rounded-[2.5rem] border border-gray-100 dark:border-coffee-800 shadow-sm overflow-hidden">
        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-8">
          {t("dashboard.analytics.customerRetention")}
        </h3>
        <div className="overflow-x-auto">
          <table className="w-full text-center text-sm">
            <thead>
              <tr className="text-gray-400 dark:text-gray-500 border-b border-gray-100 dark:border-coffee-800">
                <th className="text-left py-4 pl-4 font-bold uppercase tracking-wider text-xs">
                  {t("dashboard.analytics.cohort")}
                </th>
                <th className="py-4 font-bold uppercase tracking-wider text-xs">
                  {t("dashboard.analytics.month")} 1
                </th>
                <th className="py-4 font-bold uppercase tracking-wider text-xs">
                  {t("dashboard.analytics.month")} 2
                </th>
                <th className="py-4 font-bold uppercase tracking-wider text-xs">
                  {t("dashboard.analytics.month")} 3
                </th>
                <th className="py-4 font-bold uppercase tracking-wider text-xs">
                  {t("dashboard.analytics.month")} 4
                </th>
                <th className="py-4 font-bold uppercase tracking-wider text-xs">
                  {t("dashboard.analytics.month")} 5
                </th>
              </tr>
            </thead>
            <tbody>
              {[
                ...
                {
                  m: t("common.months.feb"),
                  retention: [100, 48, 40, 38, null],
                },
                {
                  m: t("common.months.mar"),
                  retention: [100, 52, 45, null, null],
                },
                {
                  m: t("common.months.apr"),
                  retention: [100, 55, null, null, null],
                },
              ].map((row, i) => (
                <tr
                  key={i}
                  className="border-b border-gray-50 dark:border-coffee-800 last:border-0 hover:bg-gray-50/50 dark:hover:bg-coffee-800/30 transition-colors"
                >
                  <td className="py-5 pl-4 text-left font-bold text-gray-900 dark:text-gray-200">
                    {row.m} 2024
                  </td>
                      ...
                      {val ? (
                        <span
                          className={`px-3 py-1.5 rounded-lg font-bold text-xs ${
                            val === 100
                              ? "bg-gray-100 text-gray-400 dark:bg-coffee-800 dark:text-gray-500"
                              : val > 50
