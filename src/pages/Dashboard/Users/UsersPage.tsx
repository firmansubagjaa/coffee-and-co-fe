...
export const UsersPage: React.FC = () => {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    if (view === "employees") {
      matchesView = ["admin", "barista", "superadmin", "data_analyst"].includes(
        user.role
      );
  ...
  const saveRoleChange = () => {
    if (editingUser && selectedRole) {
      ...
      toast.success(
        t("dashboard.users.toast.updated", { name: editingUser.name }),
        {
          description: t("dashboard.users.toast.desc", {
    ...
    >
            ...
            <Select value={roleFilter} onValueChange={setRoleFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="barista">
                  {t("common.roles.barista")}
                </SelectItem>
                <SelectItem value="data_analyst">
                  {t("common.roles.data_analyst")}
                </SelectItem>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-coffee-50/50 dark:bg-coffee-800/50 text-coffee-500 dark:text-coffee-400 font-bold uppercase tracking-wider border-b border-coffee-100 dark:border-coffee-800">
              <tr>
                ...
                <th className="px-6 py-4">
                  {t("dashboard.users.table.location")}
                </th>
                <th className="px-6 py-4">
                  {t("dashboard.users.table.lastActive")}
                </th>
                ...
                filteredUsers.map((user) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10 border border-coffee-100 dark:border-coffee-700">
                          <AvatarImage
                            src={`https://ui-avatars.com/api/?name=${user.name}&background=${user.avatarColor}&color=fff`}
                          />
                    ...
                    <td className="px-6 py-4">
                      <Badge
                        variant="outline"
                        className="capitalize bg-white dark:bg-coffee-800 text-coffee-700 dark:text-coffee-300 border-coffee-200 dark:border-coffee-700"
                      >
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        ...
                        {t(
                          ("common.userStatus." +
                            user.status.toLowerCase()) as any
                        )}
                      </span>
                    </td>
                      ...
                      {canEditUser(user) && (
                        ...
                        >
                          <span className="text-xs font-medium hidden group-hover:block">
                            {t("dashboard.users.table.editRole")}
                          </span>
      ...
      >
        <DialogContent>
          ...
          <div className="py-4">
            <label className="text-sm font-medium text-coffee-700 dark:text-coffee-300 mb-2 block">
              {t("dashboard.users.dialog.selectLabel")}
            </label>
            <Select
              value={selectedRole || ""}
            ...
            >
              <SelectTrigger className="bg-white dark:bg-coffee-800 border-coffee-200 dark:border-coffee-700 text-coffee-900 dark:text-white">
                <SelectValue
                  placeholder={t("dashboard.users.dialog.selectPlaceholder")}
                />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="customer">
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setEditingUser(null)}>
              {t("dashboard.users.dialog.cancel")}
            </Button>
