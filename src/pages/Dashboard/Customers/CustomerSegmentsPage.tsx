    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <button
          ...
          className={`p-4 rounded-2xl border text-left transition-all ${
            selectedSegment === "All"
              ? "bg-coffee-900 dark:bg-coffee-700 text-white border-coffee-900 dark:border-coffee-700 shadow-md"
              : "bg-white dark:bg-coffee-900 border-coffee-100 dark:border-coffee-800 hover:border-coffee-300 dark:hover:border-coffee-600"
          }`}
        >
          ...
          >
            {t("dashboard.customers.totalDatabase")}
          </p>
        </button>

        {(Object.keys(SEGMENT_INFO) as Array<keyof typeof SEGMENT_INFO>).map(
          (key) => {
            ...
            return (
              <button
                ...
                className={`p-4 rounded-2xl border text-left transition-all relative overflow-hidden group ${
                  isSelected
                    ? "ring-2 ring-offset-2 ring-coffee-900 dark:ring-coffee-400 border-transparent shadow-md"
                    : "bg-white dark:bg-coffee-900 border-coffee-100 dark:border-coffee-800 hover:shadow-sm"
                }`}
              >
                <div
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-start mb-2">
                    <span className={`font-bold ${info.color}`}>
                      {t(
                        `dashboard.customers.segments.${getSegmentKey(
                          key
                        )}.name`
                      )}
                    </span>
                    <div
                      className={`p-1.5 rounded-lg ${info.bg} ${info.color}`}
                    >
                  ...
                  <p className="text-xs text-coffee-500 dark:text-coffee-400 mt-1 truncate">
                    {t(
                      `dashboard.customers.segments.${getSegmentKey(key)}.desc`
                    )}
                  </p>
                </div>
              </button>
      ...
      <div className="bg-white dark:bg-coffee-900 rounded-[2rem] border border-coffee-100 dark:border-coffee-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-coffee-100 dark:border-coffee-800 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          {selectedSegment !== "All" && (
            <Button
              onClick={() => handleEmail(selectedSegment)}
            ...
            <thead className="bg-coffee-50/50 dark:bg-coffee-800/50 text-coffee-500 dark:text-coffee-400 font-bold uppercase tracking-wider">
              <tr>
                <th className="px-6 py-4">
                  {t("dashboard.customers.table.customer")}
                </th>
                <th className="px-6 py-4">
                  {t("dashboard.customers.table.segment")}
                </th>
                <th className="px-6 py-4">
                  {t("dashboard.customers.table.totalSpent")}
                </th>
                <th className="px-6 py-4">
                  {t("dashboard.customers.table.orders")}
                </th>
                <th className="px-6 py-4">
                  {t("dashboard.customers.table.lastVisit")}
                </th>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <Badge
                        variant="outline"
                        className={`${segment.bg} ${segment.color} border-0`}
                      >
                        {t(
                          `dashboard.customers.segments.${getSegmentKey(
                            customer.segment
