            ...
            <thead className="bg-coffee-50/50 dark:bg-coffee-800/50 text-coffee-500 dark:text-coffee-400 font-bold uppercase tracking-wider border-b border-coffee-100 dark:border-coffee-800">
              <tr>
                <th className="px-6 py-4">
                  {t("dashboard.inventory.itemDetails")}
                </th>
                <th className="px-6 py-4">
                  {t("dashboard.inventory.category")}
                </th>
                <th className="px-6 py-4">
                  {t("dashboard.inventory.stockLevel")}
                </th>
                <th className="px-6 py-4">{t("dashboard.inventory.value")}</th>
                <th className="px-6 py-4">
                  {t("dashboard.inventory.supplier")}
                </th>
                ...
                filteredItems.map((item) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-coffee-400 font-mono">
                        {item.sku}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-coffee-900 dark:text-white">
                          {item.stockLevel}{" "}
                          <span className="text-xs font-normal text-coffee-400">
                            {item.unit}
                          </span>
                        </span>
                        {item.stockLevel <= item.parLevel && (
                          <span className="text-xs text-error font-bold">
                            {t("dashboard.inventory.parLevel", {
                              level: item.parLevel,
                            })}
                          </span>
                        )}
                      ...
                      <div className="w-24 h-1.5 bg-coffee-100 dark:bg-coffee-800 rounded-full mt-1.5 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(
                              (item.stockLevel / (item.parLevel * 2)) * 100,
                              100
                            )}%`,
                          }}
                        ></div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuSeparator />
                          <DropdownMenuItem>
                            {t("dashboard.inventory.actions.viewHistory")}
                          </DropdownMenuItem>
                          <DropdownMenuItem>
                            {t("dashboard.inventory.actions.editItem")}
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </td>
