        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-coffee-500 dark:text-coffee-400 mt-1">{t('dashboard.locations.subtitle')}</p>
                </div>
                <Button onClick={() => navigate('/dashboard/cms/locations/new')} className="gap-2 shadow-lg">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-coffee-50/50 dark:bg-coffee-800/50 text-coffee-500 dark:text-coffee-400 font-bold uppercase tracking-wider border-b border-coffee-100 dark:border-coffee-800">
                            <tr>
                                <th className="px-6 py-4">{t('dashboard.locations.table.storeName')}</th>
                                <th className="px-6 py-4">{t('dashboard.locations.table.address')}</th>
                                <th className="px-6 py-4">{t('dashboard.locations.table.contact')}</th>
                                <th className="px-6 py-4">{t('dashboard.locations.table.status')}</th>
                                <th className="px-6 py-4 text-right">{t('dashboard.locations.table.actions')}</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-coffee-50 dark:divide-coffee-800">
                            {filteredLocations.map(loc => (
                                <tr key={loc.id} className="hover:bg-coffee-50/30 dark:hover:bg-coffee-800/30 transition-colors">
                                    <td className="px-6 py-4">
                                        <p className="font-bold text-coffee-900 dark:text-white">{loc.name}</p>
                                        <div className="flex items-center gap-1 text-xs text-coffee-500 dark:text-coffee-400 mt-1">
                                            <Clock className="w-3 h-3" /> {loc.hours}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-coffee-700 dark:text-coffee-300 max-w-xs truncate" title={loc.address}>{loc.address}</td>
                                    <td className="px-6 py-4 text-coffee-600 dark:text-coffee-400 flex items-center gap-1">
                                        <Phone className="w-3 h-3" /> {loc.phone}
                                    </td>
                                    <td className="px-6 py-4">
                                        <Badge variant={loc.status === 'Open' ? 'success' : loc.status === 'Renovation' ? 'warning' : 'neutral'}>
                                            {loc.status === 'Open' ? t('dashboard.locations.status.open') : loc.status === 'Renovation' ? t('dashboard.locations.status.renovation') : t('dashboard.locations.status.comingSoon')}
                                        </Badge>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => navigate(`/dashboard/cms/locations/${loc.id}`, { state: loc })} className="p-2 text-coffee-400 dark:text-coffee-500 hover:text-coffee-900 dark:hover:text-white hover:bg-coffee-100 dark:hover:bg-coffee-800 rounded-lg">
                                                <Edit2 className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleDelete(loc.id)} className="p-2 text-coffee-400 dark:text-coffee-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg">
