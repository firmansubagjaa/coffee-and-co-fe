...
export const RecipeManager: React.FC<RecipeManagerProps> = ({ variants, onUpdate }) => {
    const { t } = useLanguage();
    ...
    if (variants.length === 0) {
        return (
            <div className="bg-gray-50 dark:bg-coffee-900/50 p-6 rounded-xl border border-dashed border-gray-300 dark:border-coffee-700 text-center">
                <p className="text-gray-500 dark:text-coffee-400">{t('dashboard.products.recipe.noVariants')}</p>
            </div>
        );
    }
        ...
        <div className="bg-white dark:bg-coffee-900 p-6 rounded-2xl border border-coffee-100 dark:border-coffee-800 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-coffee-100 dark:border-coffee-800 pb-4">
                <div>
                    ...
                    <p className="text-sm text-coffee-500 dark:text-coffee-400">{t('dashboard.products.recipe.subtitle')}</p>
                </div>
                <div className="bg-green-50 dark:bg-green-900/20 px-4 py-2 rounded-xl border border-green-100 dark:border-green-800 flex flex-col items-end">
                    <span className="text-xs text-green-600 dark:text-green-400 font-medium uppercase tracking-wider">{t('dashboard.products.recipe.estimatedCost')}</span>
                    <span className="font-mono font-bold text-xl text-green-700 dark:text-green-300 leading-none">
                        {CURRENCY}{currentRecipe ? calculateCost(currentRecipe).toFixed(2) : '0.00'}
                    </span>
                </div>
            </div>
            ...
            <div className="space-y-6">
                {/* Variant Tabs */}
                <div>
                    <Label className="mb-2 block text-xs uppercase text-coffee-400 font-bold tracking-wider">{t('dashboard.products.recipe.selectVariant')}</Label>
                ...
                <div className="bg-gray-50/50 dark:bg-coffee-800/30 rounded-xl border border-coffee-100 dark:border-coffee-800 p-1">
                    <div className="bg-white dark:bg-coffee-900 rounded-lg shadow-sm border border-coffee-50 dark:border-coffee-800 overflow-hidden">
                        ...
                        <div className="grid grid-cols-12 gap-4 px-4 py-3 bg-coffee-50/30 dark:bg-coffee-800/50 border-b border-coffee-100 dark:border-coffee-800 text-xs font-bold text-coffee-500 dark:text-coffee-400 uppercase tracking-wider">
                            <div className="col-span-6">{t('dashboard.products.recipe.table.ingredient')}</div>
                            <div className="col-span-3">{t('dashboard.products.recipe.table.quantity')}</div>
                            <div className="col-span-2">{t('dashboard.products.recipe.table.unit')}</div>
                            <div className="col-span-1 text-center">{t('dashboard.products.recipe.table.action')}</div>
                        </div>
                        ...
                        <div className="divide-y divide-coffee-50 dark:divide-coffee-800">
                            {currentRecipe?.ingredients.map((ing, idx) => {
                                const item = MOCK_INVENTORY.find(i => i.id === ing.inventoryId);
                                return (
                                    <div key={idx} className="grid grid-cols-12 gap-4 px-4 py-3 items-center hover:bg-coffee-50/30 dark:hover:bg-coffee-800/30 transition-colors group">
                                        <div className="col-span-6">
                                            <Select 
                                                value={ing.inventoryId} 
                                                onValueChange={(val) => updateIngredient(idx, 'inventoryId', val)}
                                            >
                                                <SelectTrigger className="border-transparent hover:border-coffee-200 dark:hover:border-coffee-700 bg-transparent hover:bg-white dark:hover:bg-coffee-800 focus:bg-white dark:focus:bg-coffee-800 transition-all h-9 text-coffee-900 dark:text-white">
                                                    <SelectValue placeholder={t('dashboard.products.recipe.placeholders.selectItem')} />
                                                </SelectTrigger>
                                                <SelectContent>
                                                    {MOCK_INVENTORY.map(item => (
                                                        <SelectItem key={item.id} value={item.id}>
                                                            <span className="font-medium text-coffee-900">{item.name}</span>
                                                            <span className="text-coffee-400 ml-2 text-xs">({CURRENCY}{item.unitCost}/{item.unit})</span>
                                                        </SelectItem>
                                                    ))}
                                                </SelectContent>
                                            </Select>
                                        ...
                                        <div className="col-span-1 text-center">
                                            <Button 
                                                ...
                                                className="h-8 w-8 p-0 text-coffee-300 hover:text-red-500 hover:bg-red-50 rounded-full opacity-0 group-hover:opacity-100 transition-all"
                                            >
                        ...
                        {currentRecipe?.ingredients.length === 0 && (
                            <div className="py-8 text-center">
                                <p className="text-sm text-coffee-400 mb-3">{t('dashboard.products.recipe.noIngredients', { variant: selectedVariant })}</p>
                                <Button 
                                    type="button" 
                                    variant="outline" 
                                    size="sm" 
                                ...
                                >
                                    <Plus className="w-4 h-4 mr-2" /> {t('dashboard.products.recipe.addFirst')}
                                </Button>
                            </div>
                        )}
                        ...
                        {currentRecipe && currentRecipe.ingredients.length > 0 && (
                            <div className="px-4 py-3 bg-gray-50 dark:bg-coffee-800/30 border-t border-coffee-100 dark:border-coffee-800">
                                ...
                                >
                                    <Plus className="w-3 h-3 mr-2" /> {t('dashboard.products.recipe.addAnother')}
                                </Button>
                            </div>
                        )}
                    </div>
                ...
                <div className="flex gap-3 items-start bg-blue-50/50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-800">
                    <AlertCircle className="w-5 h-5 text-blue-500 dark:text-blue-400 shrink-0 mt-0.5" />
                    <div className="text-sm text-blue-700 dark:text-blue-300">
                        <p className="font-bold mb-1">{t('dashboard.products.recipe.info.title')}</p>
                        <p className="text-blue-600/80 dark:text-blue-300/80 leading-relaxed">
