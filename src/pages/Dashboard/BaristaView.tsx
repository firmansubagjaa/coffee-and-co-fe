...
const MOCK_ACTIVE_ORDERS: KDSOrder[] = [
  {
    ...
    items: [
       { id: '1', name: 'Espresso Romano', quantity: 1, category: 'coffee', modifiers: ['Extra Shot', 'Oat Milk'] },
       { id: '3', name: 'Almond Croissant', quantity: 2, category: 'pastry', notes: 'Warmed up' }
    ],
    status: 'preparing',
    total: 12.5,
    userId: 'u1',
  ...
  },
  {
    ...
    items: [
       { id: '5', name: 'Caramel Macchiato', quantity: 1, category: 'coffee', modifiers: ['Large', 'Skim Milk', 'Less Ice'] }
    ],
    status: 'pending',
    total: 5.75,
    userId: 'u2',
  ...
  },
  {
    ...
    items: [
       { id: '6', name: 'Avocado Toast', quantity: 1, category: 'food', modifiers: ['Add Poached Egg'] },
  ...
  },
  {
    ...
    items: [
        { id: '8', name: 'Matcha Latte', quantity: 1, category: 'coffee' }
    ],
    status: 'ready',
    total: 4.50,
    userId: 'u4',
...
const OrderTimer = ({ startTime, initialElapsed }: { startTime: string, initialElapsed: number }) => {
    ...
    if (elapsed >= 5) colorClass = 'text-yellow-700 bg-yellow-50 border-yellow-200';
    if (elapsed >= 10) colorClass = 'text-red-600 bg-red-50 border-red-200 animate-pulse';

    return (
        <div className={cn("px-3 py-1 rounded-lg border text-sm font-mono font-bold flex items-center gap-2", colorClass)}>
...
export const BaristaView: React.FC = () => {
  ...
  const updateStatus = (orderId: string, nextStatus: OrderStatus) => {
    setOrders(prev => {
        const newOrders = prev.map(o => o.id === orderId ? { ...o, status: nextStatus } : o);
  ...
  const avgTime = Math.round(orders.reduce((acc, curr) => acc + curr.elapsedMinutes, 0) / (orders.length || 1));
    ...
    >
      ...
      <header className="bg-white dark:bg-coffee-900 p-4 rounded-2xl border border-coffee-100 dark:border-coffee-800 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 sticky top-0 z-10">
        <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div>
                <h1 className="text-xl font-bold text-coffee-900 dark:text-white">{t('dashboard.barista.kds')}</h1>
                <p className="text-xs text-coffee-500 dark:text-coffee-400 font-medium uppercase tracking-wider">
                    {new Date().toLocaleDateString(language === 'id' ? 'id-ID' : 'en-US')} • {orders.length} {t('dashboard.barista.activeTickets')}
                </p>
            </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
            <div className="flex items-center gap-2 px-4 py-2 bg-coffee-50 dark:bg-coffee-800 rounded-lg border border-coffee-100 dark:border-coffee-700 w-full sm:w-auto justify-center sm:justify-start">
                <span className="text-xs text-coffee-500 dark:text-coffee-400 uppercase font-bold">{t('dashboard.barista.avgTktTime')}:</span>
                <span className={cn("font-mono font-bold text-lg", avgTime > 8 ? "text-red-500" : "text-coffee-900 dark:text-white")}>
                    {avgTime}m
                </span>
            </div>
                ...
                {['all', 'pending', 'preparing', 'ready'].map((f) => (
                    ...
                    >
                        ...
                        {f === 'pending' && pendingCount > 0 && (
                            <span className="ml-2 bg-red-500 text-white text-[10px] px-1.5 rounded-full">{pendingCount}</span>
                        )}
                    </button>
                ))}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 flex-1 overflow-y-auto pb-20">
                ...
                return (
                    <motion.div 
                        ...
                        className={cn(
                            ...
                            isLate ? "border-red-100 dark:border-red-900/50 shadow-red-100" : "border-coffee-50 dark:border-coffee-800"
                        )}
                    >
                        {/* Ticket Header */}
                        <div className={cn(
                            "p-4 flex justify-between items-start border-b",
                            order.status === 'ready' ? "bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-900/30" : 
                            order.status === 'preparing' ? "bg-yellow-50 dark:bg-yellow-900/20 border-yellow-100 dark:border-yellow-900/30" : 
                            "bg-gray-50 dark:bg-coffee-800/50 border-gray-100 dark:border-coffee-700"
                        )}>
                            <div>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-2xl font-black text-coffee-900 dark:text-white">#{order.id.slice(-3)}</span>
                                    <span className="text-xs font-bold text-coffee-400 uppercase tracking-wider">{t(('common.orderStatus.' + order.status) as any)}</span>
                                </div>
                                <p className="text-sm font-bold text-coffee-700 dark:text-coffee-300 truncate max-w-[150px]">{order.customerName}</p>
                            </div>
                            <OrderTimer startTime={order.date} initialElapsed={order.elapsedMinutes} />
                        </div>
                            ...
                            {order.items.map((item, idx) => (
                                <div key={idx} className="flex gap-3">
                                    ...
                                    <div className="flex-1">
                                        <p className="font-bold text-gray-800 dark:text-gray-200 text-lg leading-tight">{item.name}</p>
                        ...
                        <div className="p-2 border-t border-gray-100 dark:border-coffee-800 bg-gray-50/50 dark:bg-coffee-800/30">
                            ...
                            {order.status === 'ready' && (
                                <button 
                                    ...
                                    className="w-full py-4 rounded-xl bg-green-100 text-green-800 border border-green-200 font-bold hover:bg-green-200 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                                >
        ...
        {filteredOrders.length === 0 && (
            <div className="col-span-full flex flex-col items-center justify-center py-20 text-coffee-300">
                ...
                <h3 className="text-xl font-bold text-coffee-400">{t('dashboard.barista.noOrders').replace('{status}', filter !== 'all' ? t(('common.orderStatus.' + filter) as any) : '')}</h3>
