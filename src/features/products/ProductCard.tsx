      ...
      <div className="p-4">
        <div className="relative aspect-square overflow-hidden rounded-[1.5rem] bg-coffee-50 dark:bg-transparent shadow-inner">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                        ...
                        >
                            <Heart className={`h-4 w-4 transition-colors ${favorite ? 'fill-red-500 text-red-500' : 'text-coffee-900 dark:text-white'}`} />
                        </motion.div>
                    </motion.button>
                  </TooltipTrigger>
                  <TooltipContent>
            ...
            <div className="absolute top-3 right-3 bg-white/90 dark:bg-[#2C1A11]/60 backdrop-blur-md px-3 py-1.5 rounded-full flex items-center gap-1.5 z-20 pointer-events-none border border-white/20 dark:border-none shadow-sm">
                ...
                <span className="text-xs font-bold text-coffee-900 dark:text-white">{product.rating}</span>
            </div>
        </div>
      </div>
      
      <div className="px-6 pb-6 pt-2 flex flex-col flex-1">
        <Link to={`/product/${product.id}`} className="block transition-colors flex-1 group-hover:text-coffee-700 dark:group-hover:text-coffee-300">
            <div className="mb-3">
                <p className="text-[10px] text-coffee-500 dark:text-white/50 uppercase tracking-[0.2em] font-bold mb-2">{product.category}</p>
                <h3 className="font-serif text-xl font-bold text-coffee-900 dark:text-white leading-tight mb-2 line-clamp-1">{product.name}</h3>
            </div>
            
            <div className="text-sm text-coffee-600 dark:text-white/70 mb-6 h-10 line-clamp-2 leading-relaxed" dangerouslySetInnerHTML={{ __html: product.description }} />
