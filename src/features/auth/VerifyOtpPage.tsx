...
export const VerifyOtpPage: React.FC = () => {
  ...
  const onSubmit = async (data: { otp: string }) => {
    ...
    if (isValid) {
        ...
        setError('otp', { type: 'manual', message: t('auth.verifyOtp.validation.invalid') });
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="space-y-4 flex flex-col items-center">
            <Label htmlFor="otp" className="sr-only">{t('auth.verifyOtp.label')}</Label>
            ...
            <Controller
                ...
                rules={{ 
                    ...
                    minLength: { value: 6, message: t('auth.verifyOtp.validation.length') } 
                }}
                render={({ field }) => (
                    <InputOTP
                        maxLength={6}
            ...
            <p className="text-xs text-coffee-500">
                {t('auth.verifyOtp.tip')} <strong className="text-coffee-900">123456</strong>
            </p>
        </div>
        ...
        <div className="text-center pt-4 flex flex-col gap-4">
             <p className="text-sm text-coffee-600">
                {t('auth.verifyOtp.resendText')} <button type="button" className="font-bold text-coffee-900 underline">{t('auth.verifyOtp.resendLink')}</button>
             </p>
             <Link to="/login" className="inline-flex items-center justify-center gap-2 text-sm font-bold text-coffee-600 hover:text-coffee-900 transition-colors">
